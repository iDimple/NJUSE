...
int currentSleep;
PUBLIC int isSleeping(struct proc * p)
{
  return p->sleep_ticks != 0;
}
PUBLIC void setSleep_ticks (int ticks, struct proc * p)
{
  p->sleep_ticks = ticks;
}
PUBLIC void adjustSleep_ticks(struct proc * p)
{
  if (p->sleep_ticks == 0) return;
  else 
  {
    p->sleep_ticks -= 1;
  }
...
PUBLIC void schedule()
{
	...
	while (!greatest_ticks) {
		for (p = &FIRST_PROC; p <= &LAST_PROC; p++) {
			if (p->p_flags == 0 &&!isSleeping(p)) {
                                
				if (p->ticks > greatest_ticks) {
					greatest_ticks = p->ticks;
					p_proc_ready = p;
                                     //   sys_printx(0,0,p->name,p);
				}
			}
       ...
       current_proc = p_proc_ready - &FIRST_PROC;
     // sys_printx(0,0,"now",p_proc_ready);
      //sys_printx(0,0,p_proc_ready->name,p_proc_ready);
}
